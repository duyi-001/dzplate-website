...
function readNewContent() {
    return new Promise((resolve, reject) => {
        fs.open(inputFilePath, 'r', (err, fd) => {
            ...
            const buffer = Buffer.alloc(1024 * 1024); // 1MB 缓冲区
            let newData = '';

            function readChunk() {
                fs.read(fd, buffer, 0, buffer.length, lastOffset, (err, bytesRead) => {
                    if (err) {
                    ...
                    if (bytesRead > 0) {
                        newData += buffer.slice(0, bytesRead).toString('utf-8');
                        lastOffset += bytesRead;
                        readChunk(); // 继续读取直到没有更多数据
                    } else {
                        fs.close(fd, () => resolve(newData));
                    }
                });
            }
